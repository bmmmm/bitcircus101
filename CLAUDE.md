...
For stable upstream feeds (our own Nextcloud, partner Nextclouds): add an entry to `calendars.json`.

For curated external events/sources (kuratierte ics-single oder ics-filtered): drop a JSON file into `calendars/external/`. Filename = source id, content = one source object (same shape as a `calendars.json` array entry). Easier to add, edit, or remove single curated events without churning the central file. `sync-events.mjs` merges both.
