...
- `scripts/live-overlay.mjs`, `scripts/cache-bust.mjs`, `scripts/smoke-live.mjs` — the deploy pipeline's file logic (overlay main→live preserving CI-generated feeds, asset-hash `?v=` cache-busting across all HTML, post-deploy health poll). Extracted from inline workflow bash so they run and test locally — covered by `tests/deploy-scripts.spec.mjs`.
...
`invite-*/` and `join-*/` are 0-second redirects to Signal groups: kept **`noindex`** and **excluded from the sitemap**, but intentionally **not** `Disallow`ed — they're shareable links. Don't re-add a robots block "for consistency"; `noindex` is the right tool for a reachable redirect.
