...
which is the place to read before changing one. The five shared `*-core.js`
modules (`ics-core.js`, `events-core.js`, `finanz-core.js`, `jobs-core.js`,
`status-core.js`) are
...
status.html                 Status page (/status): which events happened, which fell through —
                            bars per week/month/quarter/year (loads status-data.json + finanz.json)
...
status.js                   Frontend: draws the /status bar rows, the zoom detail, the incident list, the pulse
status-core.js              Shared timeline math for /status (bucket ladder, states, pulse months); unit-tested
...
finanz-core.js              Shared funding math (percentages, ASCII bars, pulse) for finanz.js
                            and the maintainer CLI
...
scripts/
  ...
  validate.mjs              Field predicates (isCalendarDate, isCleanHttpsUrl) shared by the
                            funding data layer and the job board gate — belongs to neither
...
key and `finanz.js` renders nothing until one exists. The track gets a time
axis through an optional `start` month (`YYYY-MM`; `levels[i]` is `start + i`
months, one level per calendar month, no gaps): `pnpm run finanz pulse <0..7>
--start YYYY-MM` once, then `pnpm run finanz pulse <0..7>` each month — a
backfill is the same command repeated from the first month. Without `start`,
`/status` shows no pulse at all.
...
- **What it does:**
  ...
  8. Runs `scripts/build-status-data.mjs` (`status-data.json` for `/status` from `events-archive.json`)
  9. Commits and pushes to `live` with retry logic
