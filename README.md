...
scripts/
  sync-events.mjs           Fetches ICS from Nextcloud, generates events-data.json + feed.xml + calendar-public.json
...
| `calendar-public.json` | `sync-events.yml` | Every 30 minutes |
...
- Loads `events-data.json` and reads `calendar-public.json` for current primary calendar URLs
- Falls back to hardcoded URLs and live ICS fetch if generated files are unavailable
...
This fetches live ICS data and writes `events-data.json`, `feed.xml`, and `calendar-public.json` to the project root.

---
# commit rules

For clean, reviewable, low-risk commits, follow these rules to get pushed:

- **Keep scope tight:** One logical change per commit (or a small set of directly related file changes).
- **Update `changelog.md` in the same commit:** Add a short, structured entry under the current release line (or `Unreleased`) with what changed and why.
- **Versioning discipline:** Keep `package.json` version aligned with release entries in `changelog.md` when cutting releases.
- **Static-first architecture:** Keep the site plain static HTML/CSS/JS; avoid framework migrations and build complexity unless explicitly decided.
- **Dependency minimalism:** Prefer zero/very few dependencies. New dependencies need a clear reason, maintenance cost check, and security check.
- **No hidden infra coupling:** Keep deploy/sync behavior transparent in repo scripts and workflows; document any pipeline changes in README/changelog.
- **Single source for config/data:** Avoid duplicated constants (for example calendar URLs); prefer generated/shared config from existing scripts.
- **GDPR-lean external sources:** Only use necessary third-party data sources, prefer privacy-friendly providers, and document what is fetched, from where, and why.
- **No secret material in commits:** Never commit tokens, private URLs, credentials, or personal data dumps.
- **Verification before merge:** Run relevant tests for changed areas and ensure CI passes before shipping.
- **Readable commit messages:** Use concise messages that explain intent and impact, not only file names.