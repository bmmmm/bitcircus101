...
finanz-core.js              Shared funding math + field predicates for finanz.js and the CLI
...
`main` and edited through the maintainer CLI. `finanz.json` feeds the cost/funding
...
- **Half-open expiry:** a posting is up from `from` through the day *before* the
  same day-of-month `months` later. If that day-of-month does not exist in the
...
- The donation is checked by hand before the merge — there is no payment
  webhook. Verwendungszweck is `JOBS-<id>`.
...
The file that actually runs is [`.forgejo/workflows/ci.yml`](.forgejo/workflows/ci.yml) — PRs live on
Forgejo, and once that directory exists Forgejo ignores `.github/workflows/` entirely, so
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) is its twin for the GitHub mirror. Keep the two in lockstep.
...
2. Honors `noindex` meta and `robots.txt` `Disallow` automatically; `exclude-paths` covers the
   Google verification stub and `donations.html`
