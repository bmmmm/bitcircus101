...
Use `feat/…` for a new feature and `fix/…` for a repair — both are short-lived and keep history clear; `main` stays the integration line that CI deploys from.
...
- Hang a note on the Pinnwand (one object in `jobs.json`, a posting, a permanent-slot entry in `karussell`, or a note in `chiffre` — never a private address in the PR; the how-to lives on [/pinnwand](https://bitcircus101.de/pinnwand))
